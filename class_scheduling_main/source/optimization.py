...
class SchedulingModel:
    ...
    def add_constraints(self):
        ...
        # Reformulated to directly set x[p,r,h,t] = 0 if size > capacity (for efficiency)
        for pair in self.pair_set:
            course_id, class_id, session = pair[0], pair[2], pair[3]
            ...
            for room in self.room_set:
                ...
                if class_size > capacity:
                    for day in self.day_set:
                        for time in self.time_set:
                            self.solver.Add(self.x[pair, room, day, time] == 0)
                ...
                for room in self.room_set:
                    for day in self.day_set:
                        for time in self.time_set:
                            ...
                            self.solver.Add(
                                self.x[pair, room, day, time] <= 1 - self.lecturer_unavailability[pair[1], day, time]
                            )
