...
def interpret_solution(model):
    ...
    for x in model.solution_vars['x']:
        ...
        schedule[(course_id, class_id, session)] = {
            'MKKode': course_id,
            'MatakuliahNama': model.class_details[course_id][class_id][session]['course_name'],
            'KelasID': class_id,
            'SesiID': session,
            'DosenID': faculty_id,
            'DosenNama': model.class_details[course_id][class_id][session]['lecturer_name'],
            'RuanganID': room_id,
            'RuanganNama': model.room_name[room_id],
            'HariNama': day,
            'JamMulai': time,
    ...
    results = {
                ...
                'ruangan_digunakan': [{'RuanganID': room, 'RuanganNama': model.room_name[room]} 
                                      for room in sorted(used_room)],
                'ruangan_tak_digunakan': [{'RuanganID': room, 'RuanganNama': model.room_name[room]} 
                                          for room in sorted(model.room_set) if room not in used_room],
